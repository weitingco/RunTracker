...
public class RunDatabaseHelper extends SQLiteOpenHelper {
	...
	/** A convenient class to wrap a sursor that returns rows from the "run" table.
	...
	public static class LocationCursor extends CursorWrapper{
		...
		public Location getLocation(){
			if(isBeforeFirst()||isAfterLast())
				return null;
			//First get the provider out so that you can use the constructor
	...
	public LocationCursor queryLastLocationForRun(long runId){
		Cursor wrapped = getReadableDatabase().query(TABLE_LOCATION, 
				...
				null, //group by
				null, //having
				COLUMN_LOCATION_TIMESTAMP+" desc", //ordered by latest first
				"1"); //limit 1; return the first one 
	...
	public long insertLocation(long runId, Location location){
		...
		cv.put(COLUMN_LOCATION_LONGTITUDE, location.getLongitude());
